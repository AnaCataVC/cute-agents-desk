 ...
 * lives in `accounts.json` and stays empty until the Configuracion tab (phase 2+) writes it.
 */
...
const ACCOUNTS_CONFIG = path.join(paths.home, 'accounts.json');
...
/** @returns {Array<{gh: string, email?: string, color?: string, folders: {path: string, depth: number}[]}>} */
function readAccountsConfig() {
  try {
    return JSON.parse(fs.readFileSync(ACCOUNTS_CONFIG, 'utf8'));
  } catch {
    return [];
  }
}
...
module.exports = { listGhAccounts, readAccountsConfig, buildAccounts };
