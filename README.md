...
```bash
npm test        # corre de un tiro los 31 que son rápidos y no necesitan un CLI real ni ventana
npm run smoke   # la ventana entera, sin agentes: 6 pestañas, 0 errores
```

`npm test` (`tools/verify-all.js`) corre los 31 `verify-*.js` que MEDIDO tardan segundos bajo
