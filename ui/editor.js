// @ts-check
/**
 * The "Editor" tab: what the agents changed and have not integrated yet, read like an IDE.
 *
 * It exists because approving a push from a card means trusting a summary. Here the diff is
 * the thing you approve, and the conflict with main is visible before you ask for a push.
 */
...
function changeTree(state, diffs) {
  const groups = Object.entries(diffs).map(([agentId, d]) => {
    ...
    const files = closed ? '' : d.files.map((f) => {
      ...
      const active = state.ideActive === id;
      return `
...
function openTabs(state) {
  if (!state.ideOpen.length) return '';
  const tabs = state.ideOpen.map((id) => {
...
function sideBySide(file) {
  ...
  const left = file.rows.map((r) => {
    ...
    return `<div class="diff-row mono" data-kind="${kind}">
      ...
      <span class="lt">${r.kind === 'add' ? '' : esc(r.ltext)}</span></div>`;
  }).join('');
  ...
  const right = file.rows.map((r) => {
    ...
    return `<div class="diff-row mono" data-kind="${kind}">
      ...
      <span class="lt">${r.kind === 'del' ? '' : esc(r.rtext)}</span></div>`;
  }).join('');

  // minmax(0,1fr) is what keeps a long line from widening its column past half the panel:
  // the rows are `white-space:pre`, so a plain 1fr would size to the longest line.
...
function unified(file) {
  return file.rows.map((r) => {
    ...
    return `<div class="diff-row mono" data-kind="${r.kind === 'ctx' ? 'ctx' : r.kind}">
      ...
      <span class="lt">${sign} ${esc(text)}</span></div>`;
  }).join('');
}
...
function fileHeader(state, diff, file) {
...
function fileFooter(diff, agentId) {
  return `
  ...
  if (state.ideActive) {
    ...
    if (file) {
      pane = fileHeader(state, diff, file)
        ...
        + fileFooter(diff, agentId);
    }
  }

  return `
  <div>
    <div class="font-display" style="font:600 15px var(--font-display);margin-bottom:4px">Editor</div>
    <div style="font:400 12px/1.6 var(--font-body);color:var(--color-dark-text-3);max-width:660px;margin-bottom:18px">
      El trabajo de cada agente antes de integrarse. Los agentes no pueden empujar por su cuenta:
      lo revisas acá y el harness hace el commit, el push y el PR con la cuenta de la carpeta.
    </div>
