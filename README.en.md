...
- **Role Differentiation & Configurable Coordinators:** The coordinator operates strictly inside the conversation mailbox (`conversations/<id>/`) orchestrating work without mutating source code, preserving its context window while supervising multiple repositories simultaneously. When creating a conversation, users can select its engine (`claude` or `agy`), model, reasoning effort, and execution mode. Worker agents execute specific implementation tasks inside isolated Git worktrees with 100% clean context, following a radial star topology (*hub-and-spoke*) where delegation never escalates privilege (*Delegation Never Escalates Privilege*).
