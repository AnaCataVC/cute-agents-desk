...
- **Fast & Hermetic Test Suite (`npm test`):**
  ...
  - All tests must maintain 100% pass rate: every verify script in `npm test` (`tools/verify-all.js`) must pass.
...
- **Delegation Never Escalates Privilege:** A coordinator may only spawn workers whose mode it could run itself. Read and plan mode are promises the harness makes about what a conversation can do to a repo, and delegation is the one path that could hand work to a process the promise never covered. The decision lives in `mayDelegate()` (`electron/read-mode.js`) and is enforced at the single spawn path in `electron/main.js`, on the coordinator's spawn-request route only — never by trusting the request, and never as a silent downgrade.
