...
(async () => {
  ...
  const repos = await scanRepos(accounts);
  ...
  const wrongAccount = await inspectRepo(process.cwd(), { gh: 'x', email: 'nunca-va-a-calzar@ejemplo.com' }, folderRoot);
  ...
  const rightAccount = await inspectRepo(process.cwd(), { gh: 'x', email: wrongAccount.email }, folderRoot);
